<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ page contentType="text/html; charset=UTF-8" %>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">

    <!-- viewport meta -->
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description" content="Get Ins - Học IOT online tốt nhất Việt Nam">
    <meta name="keywords"
          content="IOT, internet of things, vạn vật kết nôi, học online, tự học, khóa học, điện tử, lập trình, arduino, vi điều khiển, get ins, getinsvn">

    <title>Get Ins - Dành cho học viên</title>

    <!-- inject:css -->
    <link rel="stylesheet" href="/webstatic/src/css/animate.css">
    <link rel="stylesheet" href="/webstatic/src/css/font-awesome.min.css">
    <link rel="stylesheet" href="/webstatic/src/css/fontello.css">
    <link rel="stylesheet" href="/webstatic/src/css/jquery-ui.css">
    <link rel="stylesheet" href="/webstatic/src/css/lnr-icon.css">
    <link rel="stylesheet" href="/webstatic/src/css/owl.carousel.css">
    <link rel="stylesheet" href="/webstatic/src/css/slick.css">
    <link rel="stylesheet" href="/webstatic/src/css/trumbowyg.min.css">
    <link rel="stylesheet" href="/webstatic/src/css/bootstrap/bootstrap.min.css">
    <link rel="stylesheet" href="/webstatic/src/style.css"/>
    <!-- endinject -->

    <!-- Favicon -->
    <link rel="icon" type="image/png" sizes="16x16" href="/webstatic/src/images/favicon-getins.png">
</head>

<body class="about-us">

<!--================================
START MENU AREA
=================================-->
<c:import url="../../common/web/menu-area.jsp"/>
<!--================================
END MENU AREA
=================================-->

<!--================================
START ABOUT HERO AREA
=================================-->
<c:import url="../../common/web/about-hero-area.jsp"/>
<!--================================
END ABOUT HERO AREA
=================================-->

<section class="about_mission">
    <br>
    <br>
    <div class="container">
        <div class="row">
            <div class="col-md col-sm-12">
                <div class="content_area">
                    <h1 class="content_area--title">Dành cho học viên của
                        <span class="highlight">Get Ins</span>
                    </h1>
                    <p>
                        Vì chúng tôi là bạn,
                    </p>
                    <p>
                        Get Ins luôn quan tâm và thấu hiểu, khi bắt đầu một cuộc hành trình khám
                        phá tri thức, nếu không có vận dụng và thực hành đều đặn thì kỹ năng sẽ dần
                        bị mai một, gần như sẽ trở về vạch xuất phát.
                    </p>
                    <p>
                        Get Ins ra đời với sứ mệnh đưa các bạn trẻ Việt Nam trở thành người có năng
                        lực cao thông qua việc học và rèn luyện công nghệ, để đến gần hơn và dẫn
                        đầu trong kỷ nguyên công nghệ số. Đó là lý do Get Ins đưa ra những chính
                        sách để giúp đỡ tối đa cho không chỉ học viên của Get Ins mà còn cho các
                        bạn trẻ yêu công nghệ trên mọi miền Tổ quốc.
                    </p>
                    <p>
                        Get Ins cam kết rằng, khi bạn theo học tại Get Ins bạn sẽ được:
                    </p>
                    <p>
                        <strong>ĐẢM BẢO </strong>
                        đầu ra sau mỗi khóa học
                    </p>
                    <p>
                        <strong>MIỄN PHÍ </strong>
                        bổ trợ kiến thức trực tiếp xuyên suốt quá trình học tập
                    </p>
                    <p>
                        <strong>TIẾT KIỆM</strong>
                        chi phí: Get Ins có chính sách hỗ trợ linh kiện cho học viên với chi phí
                        thấp nhất, cam kết thấp hơn giá thị trường từ 10-20%
                    </p>
                    <p>
                        <strong>HỖ TRỢ </strong>
                        xây dựng và phát triển ý tưởng công nghệ của riêng bạn, đưa ra hướng đi và
                        lời khuyên cho từng ý tưởng
                    </p>
                    <p>
                        <strong>CỘNG ĐỒNG </strong>
                        công nghệ giải đáp thắc mắc 24/7
                    </p>
                    <p>
                        <em><u>ĐIỀU KIỆN ÁP DỤNG CHÍNH SÁCH</u></em>
                    </p>
                    <p>
                        Với chính sách trên Get Ins mong muốn tạo ra môi trường học tập thuận lợi
                        nhất cho học viên của mình. Tuy nhiên, sức “Ì” là rất lớn, vì vậy, để được
                        hưởng chính sách, học viên cần đáp ứng điều kiện dưới đây.
                    </p>
                    <p>
                        <strong><em><u>Điều kiện 1</u></em></strong>
                        : Hoàn thành học phí khóa học
                    </p>
                    <p>
                        Học viên sẽ được hưởng các chính sách nêu trên trong trường hợp hoàn thành
                        đầy đủ học phí khóa học trước khi kết thúc 50% thời lượng khóa học.
                    </p>
                    <p>
                        <strong><em><u>Điều kiện 2</u></em></strong>
                        : Về việc tham gia lớp học
                    </p>
                    <p>
                        - Tham gia trên 80% thời lượng khóa học
                    </p>
                    <p>
                        - Hoàn thành trên 85% lượng bài tập được giao
                    </p>
                    <p>
                        <strong><em><u>* Lưu ý:</u></em></strong>
                    </p>
                    <p>
                        - Trong trường hợp học viên không theo học tiếp vì lí do công tác hay học
                        tập thì cần làm đơn xin bảo lưu và phả học lại từ đầu khóa học. Phí bảo
                        lưu: 100.000đ, thời hạn bảo lưu: 6 tháng kể từ ngày bảo lưu.
                    </p>
                    <p>
                        - Số tiền thanh toán sẽ không được hoàn trả hay chuyển đổi dịch vụ/hàng hóa
                        với bất kỳ trường hợp nào.
                    </p>
                    <p>
                        - Học viên giữ kỹ phiếu thu trong suốt quá trình học và xuất phiếu thu vào
                        buổi khai giảng của khóa học.
                    </p>
                </div>
            </div>
            <!-- end /.col-md-5 -->
        </div>
        <!-- end /.row -->
    </div>
    <!-- end /.container -->
    <!-- end /.about -->
    <br>
    <br>
</section>

<!--================================
START CALL TO ACTION AREA
=================================-->
<c:import url="../../common/web/call-to-action.jsp"/>
<!--================================
END CALL TO ACTION AREA
=================================-->

<!--================================
START FOOTER AREA
=================================-->
<c:import url="../../common/web/footer-area.jsp"/>
<!--================================
    END FOOTER AREA
=================================-->

<!--//////////////////// JS GOES HERE ////////////////-->

<!-- inject:js -->
<script src="/webstatic/src/js/vendor/jquery/jquery-1.12.3.js"></script>
<script src="/webstatic/src/js/vendor/jquery/popper.min.js"></script>
<script src="/webstatic/src/js/vendor/jquery/uikit.min.js"></script>
<script src="/webstatic/src/js/vendor/bootstrap.min.js"></script>
<script src="/webstatic/src/js/vendor/chart.bundle.min.js"></script>
<script src="/webstatic/src/js/vendor/grid.min.js"></script>
<script src="/webstatic/src/js/vendor/jquery-ui.min.js"></script>
<script src="/webstatic/src/js/vendor/jquery.barrating.min.js"></script>
<script src="/webstatic/src/js/vendor/jquery.countdown.min.js"></script>
<script src="/webstatic/src/js/vendor/jquery.counterup.min.js"></script>
<script src="/webstatic/src/js/vendor/jquery.easing1.3.js"></script>
<script src="/webstatic/src/js/vendor/owl.carousel.min.js"></script>
<script src="/webstatic/src/js/vendor/slick.min.js"></script>
<script src="/webstatic/src/js/vendor/tether.min.js"></script>
<script src="/webstatic/src/js/vendor/trumbowyg.min.js"></script>
<script src="/webstatic/src/js/vendor/waypoints.min.js"></script>
<script src="/webstatic/src/js/dashboard.js"></script>
<script src="/webstatic/src/js/main.js"></script>
<script src="//maps.googleapis.com/maps/api/js?key=AIzaSyBeySPFGz7DIUTrReCRQT6HYaMM0ia0knA"></script>
<script src="/webstatic/src/js/map.js"></script>
<!-- endinject -->
</body>

</html>