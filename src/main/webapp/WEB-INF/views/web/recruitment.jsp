<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ page contentType="text/html; charset=UTF-8" %>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">

    <!-- viewport meta -->
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description" content="Get Ins - Học IOT online tốt nhất Việt Nam">
    <meta name="keywords"
          content="IOT, internet of things, vạn vật kết nôi, học online, tự học, khóa học, điện tử, lập trình, arduino, vi điều khiển, get ins, getinsvn">

    <title>Get Ins - Tuyển dụng</title>

    <!-- inject:css -->
    <link rel="stylesheet" href="/webstatic/src/css/animate.css">
    <link rel="stylesheet" href="/webstatic/src/css/font-awesome.min.css">
    <link rel="stylesheet" href="/webstatic/src/css/fontello.css">
    <link rel="stylesheet" href="/webstatic/src/css/jquery-ui.css">
    <link rel="stylesheet" href="/webstatic/src/css/lnr-icon.css">
    <link rel="stylesheet" href="/webstatic/src/css/owl.carousel.css">
    <link rel="stylesheet" href="/webstatic/src/css/slick.css">
    <link rel="stylesheet" href="/webstatic/src/css/trumbowyg.min.css">
    <link rel="stylesheet" href="/webstatic/src/css/bootstrap/bootstrap.min.css">
    <link rel="stylesheet" href="/webstatic/src/style.css"/>
    <!-- endinject -->

    <!-- Favicon -->
    <link rel="icon" type="image/png" sizes="16x16" href="/webstatic/src/images/favicon-getins.png">
</head>

<body class="about-us">

<!--================================
START MENU AREA
=================================-->
<c:import url="../../common/web/menu-area.jsp"/>
<!--================================
END MENU AREA
=================================-->

<!--================================
START ABOUT HERO AREA
=================================-->
<c:import url="../../common/web/about-hero-area.jsp"/>
<!--================================
END ABOUT HERO AREA
=================================-->

<section class="about_mission">
    <br>
    <br>
    <div class="container">
        <div class="row">
            <div class="col-md col-sm-12">
                <div class="content_area">
                    <h1 class="content_area--title">Thông tin tuyển dụng của
                        <span class="highlight">Get Ins</span>
                    </h1>
                    <br>
                    <br>
                    <p>
                        GET INS được tạo ra với mục đích mang đến một môi trường trao đổi, học hỏi
                        kiến thức, kinh nghiệm về lĩnh vực kĩ thuật mà đặc biệt là IOT dành cho các
                        bạn trẻ, biến IOT từ một điều tưởng chừng như xa vời trở nên gần gũi và
                        thiết thực với đời sống thường ngày hơn bao giờ hết.
                    </p>
                    <p>
                        <strong>1. </strong>
                        <strong>Mô tả công việc:</strong>
                    </p>
                    <p>
                        a) Đứng lớp giảng dạy
                    </p>
                    <p>
                        b) Xây dựng bài giảng, học cụ, và các hoạt động tổ chức lớp học và cho học
                        viên.
                    </p>
                    <p>
                        c) Thực hiện các công tác quản lý học sinh, quản lý lớp học, từng giờ dạy
                        sao cho đúng lộ trình, giáo trình đã được kiểm duyệt.
                    </p>
                    <p>
                        d) Nhận xét quá trình học tập – tiến bộ của học viên thông qua các buổi học
                    </p>
                    <p>
                        e) Trực tiếp dạy bổ trợ cho các học sinh khi cần thiết.
                    </p>
                    <p>
                        f) Hỗ trợ các công việc liên quan đến mảng đào tạo online cùng các phòng
                        ban khác, tham gia quá trình phối hợp với bộ phận tuyển sinh để tổ chức và
                        chốt học sinh mới nhập học, cũng như quá trình giữ và phát triển các học
                        viên cũ.
                    </p>
                    <p>
                        g) Thực hiện các công việc khác khi có yêu cầu.
                    </p>
                    <p>
                        <strong>2. </strong>
                        <strong>Yêu cầu:</strong>
                    </p>
                    <p>
                        a) Có kiến thức nền tảng về: C/ C++ , html, Css, SQL
                    </p>
                    <p>
                        b) Có kinh nhiệm lập trình vi điều khiển
                    </p>
                    <p>
                        c) Đã từng Xây dựng sản phẩm IOT, sử dụng các module wifi, bluetooth,
                        truyền nhận dữ liệu lên server, điện thoại,…
                    </p>
                    <p>
                        d) Có kiến thức về điện tử tương tự là một lợi thế
                    </p>
                    <p>
                        e) Có tinh thần học hỏi, trách nhiệm với công việc
                    </p>
                    <p>
                        f) Hòa đồng, nhiệt tình
                    </p>
                    <p>
                        g) Có kĩ năng sư phạm là một lợi thế
                    </p>
                    <p>
                        <strong>3. </strong>
                        <strong>Quyền lợi:</strong>
                    </p>
                    <p>
                        a) Thu nhập từ 70.000 VND - 120.000 VND/1h
                    </p>
                    <p>
                        b) Có cơ hội được thăng tiến lên các vị trí quản lý
                    </p>
                    <p>
                        c) Được training về kĩ năng nghiệp vụ
                    </p>
                    <p>
                        d) Được đào tạo để nâng cao kiến thức chuyên môn
                    </p>
                    <p>
                        e) Phát triển và hoàn thiện bản thân trong môi trường khởi nghiệp năng
                        động, sáng tạo
                    </p>
                    <p>
                        <strong>4. </strong>
                        <strong>Thời gian làm việc:</strong>
                    </p>
                    <p>
                        a) Đảm bảo thời gian Đứng lớp (chiều, tối) và trả lời các câu hỏi của học
                        viên
                    </p>
                    <p>
                        b) Thời gian làm việc tối thiểu: 8h/tuần
                    </p>
                    <p>
                        CV xin gửi về địa chỉ email: <strong>getinsvietnam@gmail.com</strong>
                    </p>
                    <p>
                        theo cú pháp: [UT_Vị trí_Họ và tên]
                    </p>
                    <p>
                        Mọi thắc mắc vui lòng liên hệ:    <strong>0973 755 035 (Hoàng Bảo Hưng), hoặc 0977679780 (Trang)</strong>
                    </p>
                </div>
            </div>
            <!-- end /.col-md-5 -->
        </div>
        <!-- end /.row -->
    </div>
    <!-- end /.container -->
    <!-- end /.about -->
    <br>
    <br>
</section>

<!--================================
START CALL TO ACTION AREA
=================================-->
<c:import url="../../common/web/call-to-action.jsp"/>
<!--================================
END CALL TO ACTION AREA
=================================-->

<!--================================
START FOOTER AREA
=================================-->
<c:import url="../../common/web/footer-area.jsp"/>
<!--================================
    END FOOTER AREA
=================================-->

<!--//////////////////// JS GOES HERE ////////////////-->

<!-- inject:js -->
<script src="/webstatic/src/js/vendor/jquery/jquery-1.12.3.js"></script>
<script src="/webstatic/src/js/vendor/jquery/popper.min.js"></script>
<script src="/webstatic/src/js/vendor/jquery/uikit.min.js"></script>
<script src="/webstatic/src/js/vendor/bootstrap.min.js"></script>
<script src="/webstatic/src/js/vendor/chart.bundle.min.js"></script>
<script src="/webstatic/src/js/vendor/grid.min.js"></script>
<script src="/webstatic/src/js/vendor/jquery-ui.min.js"></script>
<script src="/webstatic/src/js/vendor/jquery.barrating.min.js"></script>
<script src="/webstatic/src/js/vendor/jquery.countdown.min.js"></script>
<script src="/webstatic/src/js/vendor/jquery.counterup.min.js"></script>
<script src="/webstatic/src/js/vendor/jquery.easing1.3.js"></script>
<script src="/webstatic/src/js/vendor/owl.carousel.min.js"></script>
<script src="/webstatic/src/js/vendor/slick.min.js"></script>
<script src="/webstatic/src/js/vendor/tether.min.js"></script>
<script src="/webstatic/src/js/vendor/trumbowyg.min.js"></script>
<script src="/webstatic/src/js/vendor/waypoints.min.js"></script>
<script src="/webstatic/src/js/dashboard.js"></script>
<script src="/webstatic/src/js/main.js"></script>
<script src="//maps.googleapis.com/maps/api/js?key=AIzaSyBeySPFGz7DIUTrReCRQT6HYaMM0ia0knA"></script>
<script src="/webstatic/src/js/map.js"></script>
<!-- endinject -->
</body>

</html>